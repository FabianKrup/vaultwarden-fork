...
| 7 | ✅ **`config.json` runtime writes** | ~~Admin panel writes config to disk via opendal~~ **Resolved:** `IMMUTABLE_CONFIG` env flag refuses admin config writes (`post_config`/`delete_config`), makes `Config::update_config` refuse **all** runtime writes (defense-in-depth), and makes `Config::load` skip reading `config.json` — config becomes env-only. | ~~Write on one replica invisible to others until restart~~ Done — see roadmap step 5. Unset = behavior unchanged. | `src/config.rs` (`load` gate + `update_config` guard + `immutable_config`), `src/api/admin.rs:798,807` |
...
| 11 | ✅ **Duo 2FA AKey** | ~~`get_duo_akey` auto-generates a random AKey and persists it to `config.json` when `_DUO_AKEY` is unset~~ **Resolved:** with `IMMUTABLE_CONFIG` + Duo enabled, boot **fails** unless `_DUO_AKEY` is set (`validate_config`). The HMAC AKey is now guaranteed identical across replicas. | ~~Each replica/boot minted a different random AKey (ignored at boot under immutable mode), so traditional Duo signatures failed when the auth request and callback landed on different replicas~~ Done. Only affects traditional Duo prompt; `duo_oidc` does not use the AKey. | `src/config.rs` (`validate_config`, `get_duo_akey`), `src/api/core/two_factor/duo.rs:262` |
...
| 4–5 | Rate limiting | ✅ **Done.** With `REDIS_URL` set, a **Redis-backed token bucket** (atomic Lua, server `TIME` clock) shares login/admin limits across the fleet keyed by IP, matching the `governor` quota (capacity = `*_max_burst`, refill 1 every `*_ratelimit_seconds`). Shared Redis connection lives in `src/redis_conn.rs` (also backs the WS backplane). On any Redis error — or when `REDIS_URL` is unset — falls back to the per-replica in-memory `governor`. Works independently of WebSockets. Under `IMMUTABLE_CONFIG` a missing `REDIS_URL` logs a startup warning, since per-replica limits and WebSocket fan-out are not HA-safe. |
...
| 11 | Duo 2FA AKey | ✅ **Done.** `IMMUTABLE_CONFIG` + Duo enabled requires `_DUO_AKEY` to be set in env; `validate_config` aborts boot otherwise, so the AKey is shared and stable across replicas. Auto-generation (and its `config.json` write) only runs in mutable single-instance mode. |
...
# new (target):
REDIS_URL=redis://...                # WebSocket backplane + rate limiting (required for multi-replica)
_DUO_AKEY=...                        # required if traditional Duo 2FA is enabled (else boot fails)
...
5. ✅ **Immutable config mode** (#7, #11) — **done.** `IMMUTABLE_CONFIG` env flag refuses admin config writes (`post_config`/`delete_config`, `src/api/admin.rs`), makes `Config::update_config` refuse all runtime writes (defense-in-depth), and makes `Config::load` skip `config.json` (`src/config.rs`), so config is env-only and identical across replicas. Unset = unchanged. **Enforced:** with Duo 2FA enabled, `validate_config` aborts boot unless `_DUO_AKEY` is set (the auto-generated AKey would otherwise differ per replica). A missing `REDIS_URL` under `IMMUTABLE_CONFIG` logs a startup warning (rate limits + WS fan-out would be per-replica).
